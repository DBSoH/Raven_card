...
// v1.1: 更新解析逻辑以支持新的NSFW档案格式
// 新格式: [角色] 角色名-NSFW档案 (不区分常态/堕落)
// 旧格式: [角色] 角色名-魔法少女-NSFW档案 / [角色] 角色名-堕落魔法少女-NSFW档案
function 解析角色条目(entry) {
  ...
  // 检测新格式NSFW档案: [角色] 角色名-NSFW档案
  const nsfwMatch = name.match(/^\[角色\]\s+(.+?)-NSFW档案$/);
  if (nsfwMatch) {
    return {
      真名: nsfwMatch[1].trim(),
  ...
  // 检测常态/堕落主体条目及示例对话: [角色] 角色名(-魔法少女|-堕落魔法少女)?(-示例对话)?
  const mainMatch = name.match(/^\[角色\]\s+(.+?)(-堕落魔法少女|-魔法少女)?(?:-(示例对话))?$/);
  if (!mainMatch) {
    return null;
  }

  const 状态后缀 = mainMatch[2];
  const 附属类型 = mainMatch[3];

  return {
    真名: mainMatch[1].trim(),
    类型: 状态后缀 === 堕落条目后缀 ? '堕落' : (状态后缀 === 常态条目后缀 ? '常态' : '基础'),
    附属类型: 附属类型 ?? (状态后缀 ? '主体' : '基础'),
    条目名: entry.name,
    魔法名: 附属类型 || 状态后缀 ? null : 提取条目魔法名(entry.content),
  };
}
...
function 构建世界书角色索引(worldbook) {
  ...
  worldbook.forEach(entry => {
    ...
    } else if (parsed.类型 === '基础') {
      // 基础角色条目（无状态后缀）
      if (parsed.附属类型 === '基础' && !role.常态条目名) {
        role.常态条目名 = parsed.条目名;
        role.常态魔法名 = parsed.魔法名;
      }
    }
...
function 同步预置角色当前状态(girl, role, 是否堕落) {
  ...
  girl.是否堕落 = 是否堕落 === true;
  girl.真名 = role.真名;
  girl.魔法名 = girl.是否堕落 === true ? role.堕落魔法名 : role.常态魔法名;
}
...
function 回滚AI对只读字段的修改(newVariables, oldVariables, roleIndex = null) {
  ...
  _.set(newStatData, '渡鸦.技能', _.cloneDeep(_.get(oldStatData, '渡鸦.技能', {})));

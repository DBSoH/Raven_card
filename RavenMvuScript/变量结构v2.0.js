...
const RavenSchema = z
  ...
  .transform(data => ({
    身份: normalizeOptionalText(data.身份),
    技能: data.技能,
  }))
  .prefault({ 身份: '', 技能: {} });
...
const MagicGirlSchema = z
  .object({
    ...
    魔力: z.coerce
      .number()
      .transform(value => clamp(value, 0, 999))
      .prefault(100),
    魔力上限: z.coerce
      .number()
      .transform(value => clamp(value, 0, 999))
...
const InputSchema = z
  .object({
    ...
    渡鸦: z.record(z.string(), z.unknown()).optional(),
...
export const Schema = InputSchema.transform(input => {
  // V5 兼容：自动清理旧版渡鸦魔力、在场角色情欲
  if (isRecord(input.渡鸦)) {
    delete input.渡鸦.魔力;
    delete input.渡鸦.魔力上限;
  }
  if (isRecord(input.在场角色)) {
    Object.values(input.在场角色).forEach(c => {
      if (isRecord(c)) {
        delete c.情欲;
        delete c.情欲值;
      }
  ...
  // V6 兼容：自动清理旧版精神极限
  if (isRecord(input.魔法少女)) {
    Object.values(input.魔法少女).forEach(girl => {
      if (isRecord(girl)) {
        delete girl.精神极限;
        delete girl.待堕落确认;
      }
...
function normalizeRaven(rawRaven) {
  ...
  return {
    身份: normalizeOptionalText(raven.身份),

...
function 回滚AI对只读字段的修改(newVariables, oldVariables, roleIndex = null) {
  ...
  // 回滚 渡鸦.身份：只允许玩家通过状态栏修改
  const newRaven = _.get(newStatData, '渡鸦');
  const oldRaven = _.get(oldStatData, '渡鸦');
  if (_.isPlainObject(newRaven) && _.isPlainObject(oldRaven)) {
    _.set(newRaven, '身份', _.get(oldRaven, '身份', ''));
  }
...
$(() => {
  errorCatched(async () => {
    ...
    if (是可用的Mvu变量(variables)) {
      await 纠正并回写指定变量('脚本加载', variables);
    }
    ...
    eventOn(tavern_events.CHARACTER_PAGE_LOADED, () => { window.location.reload(); });
  })();
});